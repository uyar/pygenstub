...
class Namespace:
    def __init__(self, type_, name, prototypes, subspaces):
        self.type_ = type_
        self.name = name
...
def _traverse_namespace(namespace, root, required_types):
    for node in root:
        if isinstance(node, ast.FunctionDef):
            ...
            if prototype != '':
                namespace.prototypes.append(prototype)
                required_types |= requires
        if isinstance(node, ast.ClassDef):
            subnamespace = Namespace('class', node.name, [], [])
            _traverse_namespace(subnamespace, node.body, required_types)
            namespace.subspaces.append(subnamespace)
