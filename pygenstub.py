...
_RE_MULTIPLE_SPACE = re.compile(r"\s+")
...
def extract_signature(docstring):
    ...
    sig = "".join(f.rawsource for f in sig_fields[0].children if f.tagname == "field_body")
    return _RE_MULTIPLE_SPACE.sub(" ", sig)
...
def get_aliases(lines):
    ...
    for line in lines:
        line = line.strip()
        if len(line) > 0 and line.startswith(_SIG_ALIAS):
            _, content = line.split(_SIG_ALIAS)
